...
Now that we understand the service's API, which will be the [PokeAPI](https://pokeapi.co/) for our example, we can start creating our project environment. We can start by using uv or pip (we'll be using uv on linux) to create a virtual environment, then installing some dependencies:
...
Once that works, we will move onto the next layer: the service layer. This is where we will use the API client to perform specific actions. In our simple example, the service layer has no state to maintain, and therefore can remain as a collection of functions rather than a class:
